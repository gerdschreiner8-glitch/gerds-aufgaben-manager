...
function HandwritingCanvas({ initialData, onSave }: { initialData?: string, onSave: (data: string) => void }) {
  ...
  }, [canvasHeight]); // Re-init when height changes

  useEffect(() => {
    if (ctxRef.current) {
      if (isEraser) {
        ctxRef.current.globalCompositeOperation = 'destination-out';
        ctxRef.current.lineWidth = 10; // Thicker eraser
      } else {
        ctxRef.current.globalCompositeOperation = 'source-over';
        ctxRef.current.strokeStyle = color;
        ctxRef.current.lineWidth = 1; // Thinner pen
      }
    }
  ...
  const draw = (e: React.PointerEvent) => {
    ...
    // Use getCoalescedEvents for smoother drawing if available
...
Antworte AUSSCHLIESSLICH im JSON-Format:
{
  ...
  const toggleTaskDone = () => {
    ...
    if (newIsDone && task.recurrence && task.recurrence !== 'none') {
      const nextDate = getNextDate(task.dueDate || getTodayString(), task.recurrence);
      const nextTask: Task = {
        ...task,
        id: generateId(),
        isDone: false,
        ...
        subtasks: task.subtasks.map(s => ({
          ...
          id: generateId(),
          isDone: false,
          todos: (s.todos || []).map(t => ({ ...t, id: generateId(), isDone: false }))
        })),
        syncStatus: 'local'
      };
  ...
  const addTodo = (subtaskId: string) => {
    const newSubtasks = task.subtasks.map((s: any) => {
      if (s.id === subtaskId) {
        return {
          ...s,
          todos: [...(s.todos || []), { id: generateId(), title: '', isDone: false }]
        };
      }
      return s;
    });
  ...
  const addSubtask = () => {
    const newSubtasks = [...task.subtasks, { id: generateId(), title: '', isDone: false, notes: '', todos: [] }];
    onSave({ ...task, subtasks: newSubtasks });
  };
        ...
        result.newTodos.forEach((todo: any) => {
          ...
          if (subtaskIndex !== -1) {
            updatedSubtasks[subtaskIndex] = {
              ...updatedSubtasks[subtaskIndex],
              todos: [...(updatedSubtasks[subtaskIndex].todos || []), { id: generateId(), title: todo.title, isDone: false }]
            };
          }
        });
      }
                          ...
                          onClick={async () => {
                            try {
                              ...
                              if (contacts && contacts.length > 0 && contacts[0].email && contacts[0].email.length > 0) {
                                onSave({ ...task, email: contacts[0].email[0] });
                              }
                            } catch (e) {
                              console.error('Contacts API error', e);
                            }
